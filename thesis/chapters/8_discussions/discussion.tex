...
\begin{align}
    D_\alpha(p,q) = \frac{1}{\alpha - 1}\log\int p^\alpha(x)q^{1-\alpha}(x)dx,\quad \alpha \in \mathbb{R}^+.\label{eq:renyi}
\end{align}
...
Equation~\eqref{eq:var_uf} is a problem known as Power-EP, solved in \citet{buiUnifyingFrameworkGaussian2017} for sparse \ac{GPs} with conjugate likelihoods.
...
The only parameters left are the hyperparameters $\btheta$, omitted in the previous equations, and can represent a real challenge.
