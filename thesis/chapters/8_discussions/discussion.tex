...
Naively using an additional set of points, without using the \citet{Titsias2009} assumption, leads to an algorithm with complexity $\mathcal{O}((N+M)^3)$, defeating the purpose of using inducing points.
To solve this problem we propose to mix variational inference and sampling.

\citet{hensmanMCMCVariationallySparse2015} shows that the optimal variational distribution of $\boldu$ is given by:
\begin{align}
    \log q^*(\boldu) = \expec{p(\boldf|\boldu)}{\log p(\boldy|\boldf)} + \log p(\boldu) + C,\label{eq:obj_hensman}
\end{align}
where $C$ is an intractable constant.
The idea is to sample from this objective and create an empirical variational distribution $q(\boldu) = \frac{1}{S}\sum_{s=1}^S \delta(\boldu,\boldu^s)$, where $\boldu^s \sim q^*(\boldu)$.
...
Note that, unlike in the \ac{CAVI} updates, we do not assume independence between $\boldu$ and $\bOmega$.
