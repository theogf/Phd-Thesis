...
We are still exploring ways to identify larger classes of functions equivalent \mycom{change this} to scale mixtures.
In particular, the connection with the \acf{MGF} of a distribution is a promising direction since we know how to sample from them \cite{ridout2009generating}\mycom{refomulate}.
...
\end{align}
The interesting part about this augmented likelihood is that although it is conditionally conjugate in $g$, $\omega$ and $n$, it is unclear how to infer $f$\mycom{say explicitely why}.
It turns out that the Gibbs sampler for this model is very simple.
We start with the full-conditional of $f$, we marginalize out $n$ and $\omega$, return to the original likelihood, which is conditionally conjugate, and perform a \textbf{collapsed step} for the Gibbs sampler with the marginalized full conditional $p(f|g,y)$.\mycom{reformulate this more clearly}
...
Similarly, we can use the second bound \mycom{refer to the right equation} to obtain a maximum-likelihood estimate on $\lambda$, given as well in Algorithm~\ref{alg:cavi_hetero}.
...
\end{align}
Given this augmented model, we can marginalize and augment as much as we want to find the most convenient form to compute the full-conditionals.\mycom{clarify}
For example, we could use just the first Gamma augmentation on the likelihood to find the collapsed conditional of $f$ and $\lambda$ but use the full augmentation for $g$.\mycom{rewrite}
...
Truncated series are not only computationally expensive but can also be biased and unstable! \mycom{justify}
...
Since the resulting posterior is proper, no problem will arise, but this can be dissatisfactory from a purely Bayesian perspective.\mycom{change that}
...
From this point, the rest of the derivations are identical for both models \mycom{reformulate}.
...
Note that, unlike in the traditional \mycom{explicit} \ac{VI} setting, we do not assume independence between $\boldu$ and $\bOmega$.
...
The idea of generalization using \ac{MGF} \mycom{refer chapter} is promising but is also limited.
