...
In particular, the simple connection with the \acf{MGF} of a distribution is a promising way as it allow working not only with continuous but discrete variables.
Examples of this are two of the augmentations of Chapter~\ref{ch:multiclass} which do not fit the framework of Chapter~\ref{ch:general}.
The \ac{MGF} is an interesting tool to create with hierarchical models since the mixture will be of the form $\sum_{i=0}^\infty e^{ux} p(x)$ or $\int e^{ux}p(x)dx$.
From there we can use the property that it's not only $\sigma(x)$ but $\sigma^n(x)$ in general.
Further examples are shown in this chapter in Sections \ref{sec:improvemulticlass} and \ref{sec:heteroscedastic}.

Another promising improvement is to identify when some augmented variables can be marginalized out while keeping the conditional conjugacy of the model.
...
Another very incomplete part of this work was to establish convergence rates (error as a function of the number of iterations) for the \ac{CAVI} algorithm.
...
In our new setting the optimal distribution is defined as $q(\boldf,\boldu,\bOmega) = p(\boldf|\boldu)q(\boldu, \bOmega)$.
...
We can do a similar approach for $q^*(\boldu,\boldf|\bOmega)$.
The only parameters left are the hyperparameters (omitted in the previous equations), which can be sampled by using the more traditional \ac{HMC} algorithm, again in a Gibbs sampling setting.
The advantage of this approach is that no gradients are necessary as all conditionals are known in closed-form and that no expectation over the log-likelihood are needed at all.
...
The main one being that the variational distribution $q(\boldf, \bOmega)$ optimized on the augmented model is not as accurate as the variational distribution $q(\boldf)$ optimized on the original model.
