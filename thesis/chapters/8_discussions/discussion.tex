...
All figures presented are reproducible by running the example provided in the GitHub repository \url{https://github.com/theogf/Phd-Thesis}.
...
Experimental results indicate that the error on the variational free energy (and variational parameters) is decreasing as $\|\VFE^{*} - \VFE^{t}\|\propto C\_0 e^{-ct}$, where $t$ is the number of iterations, but we did not manage to write a formal proof.
...
To confirm this trend, we would need further experiments, on real datasets and a higher number of classes. 
