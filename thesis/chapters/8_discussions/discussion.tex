...
Another model, still in a draft process, is the heteroscedastic regression model \cite{wangGaussianProcessRegression2012,lazaro2011variational}.
...
We start with the full-conditional $p(f|y,\omega,n,g,\lambda)$, marginalizing out $n$ and $\omega$ returns the original model, which is conditionally conjugate.
We get the \textbf{collapsed conditional} $p(f|y,g,\lambda)$ which is nothing else but the conditional of our original model~\eqref{eq:heteroscedastic}.
In a Gibbs sampling scheme, this allows us to perform a \textbf{collapsed step}.
A collapsed step is even beneficial
