...
The \ac{MGF} is also an interesting tool to create hierarchical models. 
...
In our setting, the optimal distribution is defined as $q^*(\boldf,\boldu,\bOmega) = p(\boldf|\boldu)q^*(\boldu, \bOmega)$, where $\bOmega$ denotes the set of augmented variables and where we also use Titsias' assumption, i.e. $p(\boldf|\boldu) = \prod_{i}p(f_i|\boldu)$.
...
When analyzing the quality of the predictive distributions, the variational distribution trained the augmented model proves to be almost as good as the variational distribution trained on the original model.
The difference of bounds mentioned earlier is not significant at convergence but will create a difference nonetheless.
