...
Here $p(\bD)$ represents the so-called evidence, and can be used to compare different model (the dependency on the used model is implicit here).
...
The posterior is used for computing all kinds of expectations of the form $\expec{p(\bZ|\bD)}{f(\bZ)} = \int f(\bZ)p(\bZ|\bD)d\bZ$.
...
Probabilistic models can roughly be divided in two categories: \textbf{generative}, i.e. observed data has been generated from a stochastic model, and \textbf{discriminative}, i.e. observed data has an input and an output, and a stochastic model connects the two.
...
Given an input $\bx \in \mathbb{R}^D$ and binary label $y\in \{ 0, 1\}$, we model the process as:
\begin{align}
...
\label{sec:posterior}
Assuming that both the prior $p(\btheta)$ and the likelihood $p(\bx|\btheta)$ are known analytically, computing the posterior \eqref{eq:bayes} in closed-form means being able to compute the integral $p(\bx)=\int p(\bx|\btheta)p(\btheta)d\btheta$.
...
\end{align*}
The latter equation can easily be normalized since it is a known distribution of the same family of the prior. \com{remove this}
...
If a conjugate prior does not exist or not desirable, a relaxation is to have all full-conditionals in closed-form, we talk then about \textbf{conditionally conjugate} models.
...
A \ac{GP} is a stochastic process $f(X)$\com{to rewrite}, where the joint distribution on any collection of variables $f_t$ follows a (multivariate) Gaussian distribution.
Linear operations on Gaussian variables can be done analytically which makes them computationally very attractive.
For instance, marginals can be computed exactly and a product of Gaussian distributions of the same variable is proportional to another Gaussian.
% The Gaussian distribution is to statistics what the harmonic oscillator is to physics".

One of the interpretations of a \ac{GP} is as a prior on functions in the \acf{RKHS}.
...
Considering a function $f$ we wish to represent with a \ac{GP}, we can project $f$ into a finite space by evaluating it on a set of data inputs $\bX = \{\bx_1, \ldots, \bx_N\}$ such that we obtain the finite-dimensional vector $\boldf$ where $f_i = f(\bx_i)$.
...
The prior on the projected \ac{GP} on $\bX$ is given by $\mathcal{N}\left(\bmu_0(\bX), K_{X}\right)$ where $\mu_0(\bX)$ is a vector function such that $\bmu_0(\bX) = \{\mu_0(\bx_i)\}_{i=1}^N$ and $K \in \mathbb{R}^{N\times N}$ is the kernel matrix, defined by $K_{ij} = k(\bx_i, \bx_j)$.
...
We have a prior on the realization of the function $\boldf$ on the data $\bX = \{\bx_i\}_{i=1}^N$, $p(\boldf) = \No(\boldf|\bmu_0, \bK)$, where $\bmu_0 = \{\mu_0(\bx_i)\}_{i=1}^N$ is the mean vector based on the function $\mu_0$.
We can add noisy observations $\by = \{y_i\}_{i=1}^N$ for each respective $\bx_i$:
\begin{align}
...
The predictive distribution of $f^*=f(\bx^*)$ on a new input $\bx^*$ can be evaluated by computing:
\begin{align}
...
The prediction for $f^*$ is a Gaussian distribution, on top of knowing the mean $m^*$ we also have a measure of uncertainty given by the variance $s^*$.
...
This tells us that for a new input $\bx^*$ observed far from the training data, i.e. potentially different from what we observed so far, the predictive uncertainty will be high.
...
Therefore, the \ac{GP} posterior obtained in the previous section is only tractable for homoscedastic \com{define this} Gaussian likelihoods, for all other cases we talk about \textbf{non-conjugate \ac{GPs}}.
...
For all these problems, the posterior is not analytically tractable, and one needs to resort to approximation methods such as the one presented in Section~\ref{sec:approx_inf}.
A strong focus of this thesis is to work with these non-conjugate likelihoods and turn them into something analytically tractable \com{change this}.
...
When computing the predictive mean and covariance, the inverse operation has a computational complexity of $\mathcal{O}(N^3)$ where $N$ is the number of samples.
...
\com{Add the nystrom approximation (Williams)}
\com{also cite Csato's thesis}
...
\end{align}
\com{Enrich this section and give details}
...
One can nonetheless approximate the posterior to obtain a useful estimator for predictions and computing function expectations of interest.
...
The advantage of sampling is its unbiasedness, i.e. in the limit of infinitely many samples one recovers the exact posterior expectation.
The collection of sampling techniques is a whole field of research and the number of methods is too large to make mention of them all in this thesis.
Therefore, the scope is restricted to methods popular with or tailored to \ac{GPs}.
We restrict the focus on \ac{MCMC} methods.
...
Gibbs sampling is a particular \ac{MCMC} method where each component of the random vector is sampled one after another.
...
We illustrate the path of the sampler on a two-dimensional bimodal example on Figure~\ref{fig:gibbs_samp}.
...
The blocked Gibbs sampler will be heavily used in the different works on augmentation as full-conditional distributions of group of variables arise naturally. \com{change tone}
...
Using Hamiltonian dynamics and augmenting the variables with an extra momentum term, one can guarantee both a high acceptance rate and a good exploration of the space.
\ac{HMC}'s proposal is generated by integrating the Hamiltonian dynamics with a randomly sampled momentum for a fixed number of steps with a given symplectic integrator.
The acceptance rate depends on the conservation of energy during the integration.
