...
When the prior is said to be \textbf{conjugate} to the likelihood, the posterior is of the same probability distribution family as the prior and can be computed analytically \cite{schlaifer1961applied}.
...
A \ac{GP} is a stochastic process $\{f_t;t\in T\}$ \mycom{be more clear about what $T$ is}, where the joint distribution on any finite collection of random variables $\{f_t\}$ follows a (multivariate) Gaussian distribution \cite{rasmussen2006gaussian}.
...
\mycom{add unification citations}
